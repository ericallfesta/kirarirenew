		...
		this.each(function() {
			...
			function draw() {
					...
					function valNameDraw(){
						...
						for(var i = 0;i < options.polygon;++i){
							// fixed to add 'no-value' class if no value is set
							// by Yuya Miyazaki at 2014.10.12
							var valName = $('<span class="'+options.valuationClass+'">'+options.valuationName[i]+'</span>').appendTo(canvasObj);
							if(input.eq(i).val() === '') valName.addClass('no-value');
